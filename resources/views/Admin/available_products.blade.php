...
<div class="container-fluid">
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            <h1 class="h3" style="color: #2d3748;">
                <i class="fas fa-check-circle" style="color: #28a745;"></i> Available Products
            </h1>
            <p style="color: #718096; margin: 0;">Products currently in stock</p>
        </div>
    ...
    <div class="row mb-4">
        <div class="col-md-12">
            <div class="card" style="border: none; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); background: linear-gradient(135deg, rgba(40, 167, 69, 0.1) 0%, rgba(40, 167, 69, 0.05) 100%);">
                <div class="card-body" style="padding: 20px;">
                    <div class="row">
                        <div class="col-md-4">
                            <div style="text-align: center;">
                                <h6 style="color: #a0aec0; margin-bottom: 10px; font-size: 12px; font-weight: 700; text-transform: uppercase;">Available Products</h6>
                                <h2 style="color: #28a745; margin: 0; font-weight: 700;">{{ $products->count() }}</h2>
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div style="border-left: 2px solid rgba(40, 167, 69, 0.2); padding-left: 20px;">
                                <p style="color: #718096; font-size: 14px; margin: 0;">
                                    <i class="fas fa-info-circle" style="color: #28a745;"></i>
                                    Ready to sell! Click <strong>Add Order</strong> to create an order.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <div class="card" style="border: none; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
        <div class="card-header" style="background: transparent; border-bottom: 1px solid #e2e8f0; padding: 20px;">
            <h5 class="mb-0" style="font-weight: 700; color: #2d3748;">
                <i class="fas fa-table"></i> In Stock Products
            </h5>
        </div>
        <div class="card-body" style="padding: 0;">
            <div class="table-responsive">
                <table class="table" id="availableProductsTable" style="margin: 0;">
                            ...
                            <tr style="border-bottom: 1px solid #e2e8f0; transition: all 0.3s ease;">
                                ...
                                <td style="padding: 15px 20px; color: #2d3748; font-weight: 600;">
                                    ...
                                    @elseif($product->stock > 0)
                                        <span style="background: rgba(255, 193, 7, 0.1); color: #ffc107; padding: 6px 12px; border-radius: 6px; font-size: 13px;">
                                            {{ $product->stock }} units <i class="fas fa-exclamation-triangle"></i> Low
                                        </span>
                                    @else
                                ...
                                <td style="padding: 15px 20px;">
                                    <div style="display: flex; gap: 8px;">
                                        <a href="{{ route('add.order', $product->id) }}" class="btn btn-sm" style="background: rgba(102, 126, 234, 0.1); color: var(--primary-color); border: none; border-radius: 6px; padding: 6px 12px; font-size: 13px; font-weight: 600; transition: all 0.3s ease;" title="Add Order">
                                            <i class="fas fa-shopping-cart"></i> Order
                                        </a>
                                        <a href="{{ route('product.edit', $product->id) }}" class="btn btn-sm" style="background: rgba(108, 92, 231, 0.1); color: #6c5ce7; border: none; border-radius: 6px; padding: 6px 12px; font-size: 13px; font-weight: 600; transition: all 0.3s ease;" title="Edit">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" style="padding: 40px 20px; text-align: center; color: #a0aec0;">
                                    <i class="fas fa-inbox"></i> No products available. <a href="{{ route('add.product') }}" style="color: var(--primary-color); text-decoration: none; font-weight: 600;">Add one now</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
